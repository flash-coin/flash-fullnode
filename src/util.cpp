...
#else
    const char* pszModule = "flashcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Flashcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Flashcoin";
#else
    // Unix
    return pathRet / ".flashcoin";
#endif
#endif
}

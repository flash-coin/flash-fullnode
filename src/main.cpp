...
#include "core_io.h"  // parseHex func
#include "bls/blocksigning.h" // BLS coinbase transaction signing
...
#if defined(NDEBUG)
# error "Flashcoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Flashcoin Signed Message:\n";
                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s FLASH additional fees, %d delta bytes\n",
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    // Check the header
    // WL re-enable check POW because TFlashcoin miner could generate block with difficulty of mining < Target
    if (!CheckProofOfWork(block.GetPoWHash(), block.nBits, consensusParams, block))
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if (halvings >= 64)
        return 0;
    CAmount nSubsidy = CONF_REWARD_COIN_VALUE * COIN; // WL
    // Subsidy is cut in half every 210,000 blocks which will occur approximately every 4 years.
    if(nHeight > CONF_NUMBER_BLOCK_HAS_REWARD) // no block reward after 9000 blocks
        nSubsidy = 0;
...
void ThreadScriptCheck() {
    RenameThread("flashcoin-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // WL disabele because InvalidChainFound: invalid block=ad6b0ea1e88d1abda8976671c20c9ccd20d62687279011cd2fe2def37b48f1f2  
    //height=178383  log2_work=20.488261  date=2017-10-19 21:25:46
    //if (!control.Wait())
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW)
{
    ...
    if (fCheckPOW && !CheckProofOfWork(block.GetPoWHash(), block.nBits, consensusParams, block))
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // WL miner signature in coinbase must be valid
	// Block timestapm from which we apply block-signing.
	// 1557032400 - Human time (GMT): Sunday, May 5, 2019 5:00:00 AM
    int64_t timestamp_R3 = 1557032400;
    if(block.vtx[0].IsCoinBase()){
        std::string strScriptSig = ScriptToAsmStr(block.vtx[0].vin[0].scriptSig, true);
        //cout << "scriptSig miner Signature: " << strScriptSig.c_str() << endl;
        if((block.GetBlockTime() > timestamp_R3)){
            if(BlsVerifyCoinbaseTransaction(strScriptSig, block.hashPrevBlock.ToString())){
                //cout << "Miner Signature is VALID: " << strScriptSig.c_str() << endl;
                LogPrintf("Miner Signature is VALID: %s\n", strScriptSig.c_str());
            }else {
                //cout << "Miner Signature is INVALID: " << strScriptSig.c_str() << endl;
                LogPrintf("Miner Signature is INVALID: %s\n", strScriptSig.c_str());
                return state.DoS(100, false, REJECT_INVALID, "bad-cb-signature-miner", false, "miner signature is not valid");
            }
        }
    }
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, CBlockIndex * const pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Flashcoin: Reject block.nVersion=1 blocks (mainnet >= 710000, testnet >= 400000, regtest uses supermajority)
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // Flashcoin: (mainnet >= 710000, testnet >= 400000, regtest uses supermajority)
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    if (nCheckDepth <= 0)
        // Flashcoin: suffices until year 10214. Didn't x4 value due to integer wrap around and upstream compatibility.

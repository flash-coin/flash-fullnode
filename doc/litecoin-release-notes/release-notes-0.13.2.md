Flashcoin Core version 0.13.2 is now available from:

  <https://download.flashcoin.io/flashcoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/flash-coin/flashcoin/issues>
...
updates will be released anymore. Without security updates, using a flashcoin
...
ECDSA signatures inside Flashcoin transactions now use validation using
...
Previous versions of Flashcoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Flashcoin Core was to raise the default minimum
relay fee.

Flashcoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Flashcoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running flashcoind without having to do any manual configuration.
...
Flashcoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Flashcoin Core will relay transactions with
...
entirely in Flashcoin Core 0.13, and it is at this time undecided whether the
...
Flashcoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Flashcoin Core automatically creates a hidden service to listen on, without
manual configuration. Flashcoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Flashcoin Core is listening, and
...
Flashcoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Flashcoin
Core to use floating fees. By default, Flashcoin Core will use floating
fees.
...
(default: `0.0002` FLASH/kB).

At all times, Flashcoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) FLASH.
Furthermore, Flashcoin Core will never create transactions paying less than
...
`flashcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /flashcoinrpc>
        ...
        # optional bypass flashcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< flashcoinrpc:<password>
    </Location>
...
Restarting flashcoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in flashcoin.conf
...
flashcoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/flashcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Flashcoin Core code base has
...
- `flashcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `flashcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Flashcoin scripting system that in combination with
...
Flashcoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Flashcoin Core.
...
The two phases are distinct in the Flashcoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Flashcoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Flashcoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Flashcoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of flashcoin they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of flashcoin being spent is exactly
...
- **Increased security for multisig:** Flashcoin addresses (both P2PKH addresses
  ...
  believed to be provided by Flashcoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Flashcoin's current ledger (called the UTXO set),
  ...
  note that Flashcoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Flashcoin users to individually opt-in to almost any change in the Flashcoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Flashcoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Flashcoin
client to Flashcoin Core 0.13.2 to signal for segwit support. When a
...
Since Flashcoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `flashcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `flashcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
